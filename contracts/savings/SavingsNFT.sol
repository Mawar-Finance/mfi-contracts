...
contract SavingsNFT is ERC721 {
    ...
    /// @dev Overrides tokenURI to return different metadata based on rose count
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_ownerOf(tokenId) != address(0), "Token does not exist");
        
        uint8 roses = _roseCount[tokenId];
        if (roses == 0) roses = 1; // fallback
        
        // Returns: https://gateway.pinata.cloud/ipfs/YOUR_CID/1.json, etc.
        return string(abi.encodePacked(_baseURI(), _toString(roses), ".json"));
    }

    /// @notice Base URI for metadata - CHANGE THIS to your IPFS CID or server URL
    function _baseURI() internal pure override returns (string memory) {
        // Put your FOLDER CID here (the CID you get when uploading the metadata folder)
        // The contract will append "1.json", "2.json", etc. to this base URI
        return "https://gateway.pinata.cloud/ipfs/bafybeihjvvdzdltksfgcq4suwarhj65w4mi7wo3svvwfgpqtdm3pnjveme/";
    }
